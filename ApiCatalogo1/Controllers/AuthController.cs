    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateRole(string roleName)
        {
            ...
            if (!roleExist)
            {
                ...
                if (roleResult.Succeeded)
                {
                    ...
                    return StatusCode(StatusCodes.Status200OK, new ResponseDTO { Status = "Sucess", Message = "Role " + roleName + " added successfully" });
                }
                else
                {
                    _logger.LogInformation(2, "Roles Error");
            ...
            return StatusCode(StatusCodes.Status400BadRequest, new ResponseDTO { Status = "Error", Message = "Role " + roleName + " exists" });
        ...
        public async Task<IActionResult> AddUserToRole(string email, string roleName)
        {
            ...
            if (user != null)
            {
                ...
                if (result.Succeeded)
                {
                    var message = "User " + user.Email + " added to the " + roleName + " role";
                    _logger.LogInformation(1, "Sucess:" + message);
                    return StatusCode(StatusCodes.Status200OK, new ResponseDTO { Status = "Sucess", Message = message });
                }
                else
                {
                    var message = "Unable to add user " + user.Email + " to the " + roleName + " role";
                    _logger.LogInformation(2, "Error:" + message);
                    return StatusCode(StatusCodes.Status400BadRequest, new ResponseDTO { Status = "Error", Message = message });
                }
            }
        ...
        public async Task<IActionResult> Login([FromBody] LoginModelDTO loginModelDTO)
        {
            ...
            if (user is not null && await _userManager.CheckPasswordAsync(user, loginModelDTO.Password!))
            {
                ...
                var refreashToken = _tokenService.GenerateRefrashToken();
        ...
        public async Task<IActionResult> Register([FromBody] RegisterModelDTO registerModelDto)
        {
            ...
            if (!result.Succeeded)
            {
                return StatusCode(StatusCodes.Status500InternalServerError, new ResponseDTO { Status = "Erro", Message = "User creation failed" });
            }
        ...
        public async Task<IActionResult> RefreshToken([FromBody] TokenModelDTO tokenModelDTO)
        {
            ...
            string? refreashToken = tokenModelDTO.RefreshToken ?? throw new ArgumentException(nameof(tokenModelDTO));
            ...
            if (user == null || user.RefreshToken != refreashToken || user.RefreshTokenExpiryTime <= DateTime.Now)
            {
                return BadRequest("Invalid access token/refresh token");
            }
            ...
            var newRefreshToken = _tokenService.GenerateRefrashToken();
